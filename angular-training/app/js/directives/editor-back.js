...
var EditorConstructor = function(editorConfig) {
	...
	return {
		...
		controller : [ '$scope', '$attrs', '$transclude', '$window', '$location', '$timeout', 'CompletionService', 'SaveService',
			function(scope, attrs, transclude, window, location, timeout, completionService, saveService) {
                ...
                opts = angular.extend({},options,scope.$eval(attrs.editorConfig));
                acee = window.ace.edit("editor");
                            ...
                            if (angular.isDefined(callback)) {
                                scope.$apply(function () {
                                    if (angular.isFunction(callback)) {
                                        ...
                                        throw new Error('functione expected as callback');
                                    }
                                });
                            }
                        }
